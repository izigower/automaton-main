  ...
  if (hasTable(db.raw, "goals")) {
    try {
      ...
      if (!process.env.OPENAI_API_KEY && config.conwayApiKey) {
        ...
        const useOllama = !config.conwayApiUrl || config.inferenceProvider === 'ollama';
        if (useOllama) {
          process.env.OPENAI_BASE_URL = config.ollamaBaseUrl || 'http://10.0.149.127:11434/v1';
          logger.info(`Using Ollama mode: ${process.env.OPENAI_BASE_URL}`);
        } else {
